...
3. `cd ./nginx-1.19.2/`
4. Run ./configure: `./configure` It tells what packages are lost in order to compile the source:
...
5. Install Missed packages
   Alpine: `apk add --no-cache make pcre pcre-dev zlib zlib-dev libressl-dev`
...
> `./configure --sbin-path=/usr/bin/nginx --conf-path=/etc/nginx/nginx.conf --error-log-path=/var/log/nginx/er ror.log --http-log-path=/var/log/nginx/access.log --with-pcre --pid-path=/var/run/nginx.pid --with-http_ssl_module`
